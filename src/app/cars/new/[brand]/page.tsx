...
function BrandNewCarsContent() {
  ...
  useEffect(() => {
    if (filters.make.toLowerCase() !== brandSlug) {
      setFilters(prev => ({ ...prev, make: brandSlug }));
    }
    setVisibleCount(6);
  }, [filters.make, brandSlug, filters.model, filters.minPrice, filters.maxPrice, filters.minYear, filters.maxYear, filters.city, filters.transmission, filters.fuelType, sortBy]);

  useEffect(() => {
    async function fetchCars() {
      try {
        const res = await fetch(`/api/new-cars?brand=${brandSlug}`);
        if (res.ok) {
  ...
  const removeFilter = (key: string) => {
    if (key === "condition" || key === "make") return;
    ...
    <>
      ...
      <main className="pt-24 pb-16 bg-gray-50 min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="font-heading font-black text-[32px] md:text-[40px] text-black mb-2.5 capitalize">{brandSlug} New Cars</h1>
            <p className="font-body font-normal text-[15px] text-gray-600">Explore the latest {brandSlug} models available</p>
          </div>
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-4">
                  ...
                  >
                    ...
                    Filters {activeFiltersCount > 2 && `(${activeFiltersCount - 2})`}
                  </button>
              ...
              {activeFiltersCount > 2 && (
                <div className="flex flex-wrap gap-2 mb-6">
                  {Object.entries(filters).map(([key, value]) => {
                    if (!value || key === "condition" || key === "make") return null;
                    return (
...
export default function BrandNewCarsPage() {
  return (
